/* --------------------------------------------------------------------------------------
   ...
   Default state of Active LOW LED is ON state

   Development boards of LPC2129 contains Active LOW onboard LEDs
...
int main()
{
	...
	IOSET0=LED;    // Initially Active LOW LEDs are in ON state so IOSET0 register sets pins to HIGH and turns OFF LEDs
	delay_ms(200);
	while(1)      // Infinite loop
	{
		IOCLR0=LED;   // Sets P0.1 P0.2 and P0.3 LOW and turns ON the LEDs
		delay_ms(500);
		IOSET0=LED;   // Sets P0.1 P0.2 and P0.3 HIGH and turns OFF the LEDs
		delay_ms(500);
	}
}
